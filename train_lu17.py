    ...
    for i in range(nr):
        for j in range(nc):
            with torch.no_grad():
                ...
                if corr.abs() >= rlim: corrmat[i, j] = corr
    return torch.nanmean(corrmat**2)
...
def main(args: argparse.Namespace):
    ...
    if method == "unet":
        model = baselines.UNetRegression(nd, dh=32, factor=1, depth=1,  **vars(args))
    if method == "ns_unet":
        model = baselines.UNetRegression(nd, dh=32, factor=1, depth=1,  **vars(args))
    ...
    ncv = 50
    cvts = np.random.choice(range(nt), ncv)
    # cvts = np.arange(nt)
    val_preds = []
    for i_t, t in enumerate(cvts):
        ...
        trainer = pl.Trainer(
            ...
            max_epochs=args.epochs*(4  if i_t==0 else 1),
        ...
        # add r2shen
        r2shen = compute_r2shen(Y[ix], model(C[ix]), M[ix])
        ...
        r2shen_val = compute_r2shen(Y[cvts[:(i_t + 1)]], Yval_hat, M[:(i_t + 1)])
        prog.update(dict(r2shen_val=r2shen_val, sample_size=i_t))
    ...
    Yval_hat = torch.stack(val_preds)
    r2shen_val = compute_r2shen(Y[cvts], Yval_hat, M[cvts])
